...
def get_feedback_loop_p_iconsts_block(serial_number):
    ...
    serial_number = c_char_p(bytes(str(serial_number), "utf-8"))
    proportionalAndIntegralConstants = PZ_FeedbackLoopConstants(serial_number)

    output = PCC_GetFeedbackLoopPIconstsBlock(serial_number, proportionalAndIntegralConstants)
    if output != 0:
        raise KinesisException(output)
    return proportionalAndIntegralConstants
...
def set_max_output_voltage(serial_number, voltage):
    # Sets the maximum output voltage.

    serial_number = c_char_p(bytes(str(serial_number), "utf-8"))
    maxVoltage = c_short(voltage)
...
PCC_SetPosition.restype = c_ushort
